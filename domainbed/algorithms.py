...
class GANERM(Algorithm):
    """
    Empirical Risk Minimization (ERM) with GAN augmentation on input images
    """
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.dataset == 'PACS':
            ...
            t_idx = int(self.batch_size / 2)

            if first_half:
                # ONLY TRANSFORM FIRST HALF OF BATCH
                x_1[:t_idx] = (x_1[:t_idx] + (self.alpha * self.gan1_2(x_1[:t_idx]))
                               + (self.beta * self.gan1_3(x_1[:t_idx])))
                x_1[:t_idx].detach()
                x_1[:t_idx] = norm(x_1[:t_idx])

                x_2[:t_idx] = (x_2[:t_idx] + (self.alpha * self.gan2_1(x_2[:t_idx]))
                               + (self.beta * self.gan2_3(x_2[:t_idx])))
                x_2[:t_idx].detach()
                x_2[:t_idx] = norm(x_2[:t_idx])

                x_3[:t_idx] = (x_3[:t_idx] + (self.alpha * self.gan3_1(x_3[:t_idx]))
                               + (self.beta * self.gan3_2(x_3[:t_idx])))
                x_3[:t_idx].detach()
